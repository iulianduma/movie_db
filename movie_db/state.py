...
class MovieState(rx.State):
    ...
    GENRE_MAP: Dict[str, int] = {"Acțiune": 28, "Comedie": 35, "Dramă": 18, "Horror": 27, "SF": 878, "Thriller": 53, "Documentar": 99, "Western": 37}
    ...
    def reset_filters(self):
        self.y_start, self.y_end, self.sort_by, self.selected_genres = "1950", "2026", "vote_average.desc", []
    ...
    async def fetch_movies(self):
        ...
        if self.show_mode.lower() in ["watchlist", "watched"]:
            ...
            params = {"api_key": os.environ.get("TMDB_API_KEY"), "language": "ro-RO", "sort_by": self.sort_by, "vote_count.gte": 150, "primary_release_date.gte": f"{self.y_start}-01-01", "primary_release_date.lte": f"{self.y_end}-12-31", "with_genres": ",".join(map(str, self.selected_genres)) if self.selected_genres else None}
            r = requests.get("https://api.themoviedb.org/3/discover/movie", params=params).json()
            raw = []
            for m in r.get("results", []):
                m_id = str(m.get("id"))
                if m_id in self.watched_ids: continue
                raw.append({"id": m_id, "title": m.get("title"), "vote_average": round(m.get("vote_average", 0), 1), "poster_path": f"https://image.tmdb.org/t/p/w500{m.get('poster_path')}", "overview": m.get("overview", "")[:200] + "...", "yt_id": ""})
        self.movies = raw
    ...
    async def load_trailer(self, m_id: str):
        res = requests.get(f"https://api.themoviedb.org/3/movie/{m_id}/videos", params={"api_key": os.environ.get("TMDB_API_KEY")}).json()
