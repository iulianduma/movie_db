        ...
        rx.vstack(
            rx.box(
                rx.cond(
                    ...
                    rx.html(
                        "<iframe width='100%' height='230' src='https://www.youtube.com/embed/" 
                        + m["yt_id"].to_string() 
                        + "?autoplay=1' frameborder='0' allowfullscreen style='border-radius: 8px 8px 0 0;'></iframe>"
                    ),
                    rx.box(
                        rx.image(
                            src=rx.cond(
...
def sidebar():
    return rx.vstack(
        ...
        rx.flex(
            rx.button("🔥", on_click=lambda: MovieState.get_by_mood("Adrenalină"), variant="soft", size="1"), 
            rx.button("☕", on_click=lambda: MovieState.get_by_mood("Relaxare"), variant="soft", size="1"), 
            rx.button("🕵️", on_click=lambda: MovieState.get_by_mood("Mister"), variant="soft", size="1"), 
            rx.button("🚀", on_click=lambda: MovieState.get_by_mood("Futuristic"), variant="soft", size="1"), 
